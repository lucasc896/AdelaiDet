...
from .utils import imrescale, center_of_mass, point_nms, mask_nms, matrix_nms, normalizer
...
class SOLOv2(nn.Module):
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        """
        images_tensor, new_images_sizes = self.preprocess_image(batched_inputs)
        ...
        if self.training:
            ...
            results = self.inference(
                pred_cates=cate_pred,
                pred_kernels=kernel_pred,
                pred_masks=mask_pred,
                cur_sizes=new_images_sizes,
    ...
    def preprocess_image(
        ...
        images = [normalizer(x=x, pixel_mean=self.pixel_mean, pixel_std=self.pixel_std) for x in images]
        images = ImageList.from_tensors(images, self.backbone.size_divisibility)
        return images.tensor, images.image_sizes
    ...
    def inference(
        self,
        pred_cates: List[torch.Tensor],
        pred_kernels: List[torch.Tensor],
        pred_masks,
    ...
    def inference_single_image(
        ...
        # process.
        inds: torch.Tensor = cate_preds > self.score_threshold
        ...
        size_trans = (
            torch.tensor(self.num_grids, dtype=cate_labels.dtype).pow(2).cumsum(0)
        )
...
class SOLOv2MaskHead(nn.Module):
    ...
    def forward(self, features: List[torch.Tensor]):
        ...
        for i, conv_level in enumerate(self.convs_all_levels):
            if i > 0:
                ...
                feature_add_all_level = feature_add_all_level + conv_level(mask_feat)
